...
class Book_basic(db.Model):
    ...
    book_basic_num = db.Column(db.Integer, nullable=False)
    book_record_num = db.Column(db.Integer, nullable=False)
    ...
    def update(self, name=False, author=False, cover=False, update_num:int=0):
    ...
    def insert_origin_books():
        ...
        if temp < 1:
            import csv, time
            start = time.clock()
            origin_books = r'C:\Users\Raibows\PycharmProjects\library\app\static\book-cover\origin-info.csv'
            books_num = 0
    ...
    def get_book_basic_status(self):
        ...
        num_all = len(temp)
        num_borrow = 0
        book_borrowed_statistic = {}
        for book in temp:
            if book.book_borrowed:
                num_borrow += 1
                book_borrowed_statistic[book.book_id] = True
            else:
                book_borrowed_statistic[book.book_id] = False
        num_remain = num_all - num_borrow
        lost_num = self.book_record_num - num_all
        return (num_all, num_remain, num_borrow, lost_num, book_borrowed_statistic)
