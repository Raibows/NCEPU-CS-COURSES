...
void MD5::MD5Init(MD5_CTX* context)//初始化4个寄存器
{
...
void MD5::MD5Update(MD5_CTX* context, unsigned char* input, unsigned int inputLen)
{
	...
	if (inputLen >= partLen)
	{
		MD5_memcpy((POINTER)&context->buffer[index], (POINTER)input, partLen);//中间的赋值给左面的
...
//压缩函数
void MD5::MD5Transform(UINT4 state[4], unsigned char block[64])//压缩函数
{
	UINT4 a = state[0], b = state[1], c = state[2], d = state[3], x[16];

	Decode(x, block, 64);//由block[64]产生x[16]
...
void MD5::MD5_memchange(unsigned char output[8], UINT4 input[2])
{
	//int count;
...
int MD5::MD5File(unsigned char* filename1, unsigned char* output)
{
...
void MD5::MD5String(unsigned char* inputData, unsigned char* hashstr)
{
	int i;
	...
	if (iInputLen < 64)
	{
		...
		iPaddingStringLen = 64 - iInputLen;//填充
		MD5_memcpy((POINTER)&mInformation->buffer[iInputLen], (POINTER)&PADDING[0], iPaddingStringLen);
	...
	else
	{
		...
		MD5_memcpy((POINTER)&mInformation->buffer[0], (POINTER)&cInputLen[72 - iInputLen], iInputLen - 64);
		iPaddingStringLen = 64 + 64 - iInputLen;//填充
		MD5_memcpy((POINTER)&mInformation->buffer[iInputLen - 64], (POINTER)&PADDING[0], iPaddingStringLen);
